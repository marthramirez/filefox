  ...
  <div class="dashboard">
    ...
    <main :class="['main-content', { 'with-sidebar': isDesktop }]">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="fw-bold text-primary mb-3">Invitations</h4>
      </div>
              ...
              >
                ...
                <td class="text-end">
                  <button
                    ...
                    @click="openAcceptInviteModal(invitation.id)"
                  >
                    Accept
                  </button>
                  <button
                    class="btn btn-danger btn-sm"
                    @click="openRejectModal(invitation.id)"
                  >
                    Reject
                  </button>
                </td>
...
export default {
    ...
    searchQuery(newVal) {
      ...
      if (!query) {
        ...
        this.filteredInvitations = this.invitations.filter(
          (invitation) =>
            (invitation.team_name || "").toLowerCase().includes(query) ||
            (invitation.owner_email || "").toLowerCase().includes(query)
        );
      }
    },
  },
  ...
  methods: {
    ...
    async loadInvitations() {
      ...
      await axios
        .post(
          `${process.env.VUE_APP_API_URL}/getUserInvites`,
          { user_id: userId },
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        .then((response) => {
          if (response.data.response === "success") {
            this.invitations = response.data.invites;
            this.filteredInvitations = this.invitations;
          } else {
    ...
    // functions to handle accepting invitations
    openAcceptInviteModal(invitationId) {
    ...
    acceptInvitation() {
      ...
      axios
        .post(
          `${process.env.VUE_APP_API_URL}/acceptInvitation`,
          {
    ...
    // functions to handle reejecting invitations
    openRejectModal(invitationId) {
      this.invitation_id = invitationId;
    ...
    async rejectInvitation() {
      ...
      await axios
        .post(
          `${process.env.VUE_APP_API_URL}/rejectInvitation`,
          {
            invitation_id: this.invitation_id,
          },
          {
