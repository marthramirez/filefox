  ...
  <div class="dashboard">
    ...
    <main :class="['main-content', { 'with-sidebar': isDesktop }]">
      ...
      >
        ...
        <p class="fs-4 fw-semibold text-center">No available files here.</p>
      </div>
        ...
        >
          <div class="folder-dropdown position-absolute top-0 end-0 m-2">
            ...
            >
              <div
                class="folder-menu-item"
                v-if="$route.params.permission == 'viewer'"
              >
                View
              </div>

              <template v-if="$route.params.permission == 'editor'">
                <div
                  class="folder-menu-item"
          ...
          <router-link
            :to="`/team/folder/contents/${folder.id}/${encodeURIComponent(
              folder.folder_name
            )}/${$route.params.team_id}/${encodeURIComponent(
              folder.permission
            )}`"
            class="text-decoration-none text-reset d-block pt-3"
          >
...
export default {
  ...
  mounted() {
    ...
    this.currentPermission = this.$route.params.permission;
  ...
  methods: {
    ...
    async openPermissionModal(item_id, item_name, item_type) {
      this.activeDocumentMenu = null;
      this.shareSubMenuIndex = null;
      this.itemInfo.name = item_name;
      this.itemInfo.owner_email = this.userEmail;
    ...
    async getFolders(folder_id) {
      ...
      await axios
        .post(
          `${process.env.VUE_APP_API_URL}/getTeamSubfolders`,
          {
            team_id: team_id,
            folder_id: folder_id,
          },
    ...
    async getDocuments(folder_id) {
      const token = localStorage.getItem("token");
      const team_id = this.$route.params.team_id;

      await axios
        .post(
          `${process.env.VUE_APP_API_URL}/getTeamFolderFiles`,
          {
            team_id: team_id,
            folder_id: folder_id,
          },
      ...
      await axios
        ...
        .then((response) => {
          if (response.data.response === "success") {
            ...
            this.showErrorModal(
              "Oops!, this file is already shared with your team."
            );
          } else {
            this.showErrorModal(
              "An error occurred while sharing the file. Please try again."
      ...
      await axios
        ...
        .then((response) => {
          if (response.data.response === "success") {
            ...
            this.showErrorModal(
              "Opps!, this folder is already shared with your team."
            );
          } else {
            this.showErrorModal(
              "An error occurred while sharing the folder. Please try again."
