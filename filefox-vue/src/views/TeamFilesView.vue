  ...
  <div class="dashboard">
    ...
    <main :class="['main-content', { 'with-sidebar': isDesktop }]">
      ...
      >
        ...
        <p class="fs-4 fw-semibold text-center">No available files.</p>
      </div>
              ...
              <template v-if="folder.permission === 'editor'">
                ...
                >
                  ...
                  >
                    <div
                      class="folder-menu-item"
                      @click="toggleShowShareToPersonModal(folder.id, 'folder')"
                    >
                      To a person
                    </div>
                    <div
...
export default {
  ...
  methods: {
    ...
    async loadPageContents() {
      ...
      try {
        await Promise.all([this.getFolders(), this.getDocuments()]);
      } catch (error) {
    ...
    async getFolders() {
      ...
      return axios
        .post(
          `${process.env.VUE_APP_API_URL}/getTeamSharedFolders`,
          { team_id: team_id },
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    async getDocuments() {
      ...
      return axios
        .post(
          `${process.env.VUE_APP_API_URL}/getTeamSharedFiles`,
          { team_id: team_id },
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    // functions to handle renaming folders
    ...
    // functions for sharing files to user
    ...
    // functions for sharing files to team
      ...
      await axios
        ...
        .then((response) => {
          if (response.data.response === "success") {
            ...
            this.showErrorModal(
              "Sorry, this folder is already shared with your team."
            );
          } else {
            this.showErrorModal(
              "An error occurred while sharing the folder. Please try again."
