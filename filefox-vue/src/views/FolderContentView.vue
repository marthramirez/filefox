  ...
  <div class="dashboard">
      ...
      <div class="search-wrapper mx-3">
        <div class="input-group">
          ...
          <input
            ...
            placeholder="Search docements, folders..."
          />
        </div>
      </div>
    ...
    <main :class="['main-content', { 'with-sidebar': isDesktop }]">
      <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="dropdown position-relative" ref="dropdown">
          <button
            ...
            @click="showDropdownAddBtn"
          >
            <i class="bi bi-plus me-1"></i> New
            <i class="bi bi-caret-down-fill ms-2 small"></i>
          </button>

          <div
            v-if="showAddBtnDropdown"
            class="dropdown-menu show mt-2"
            style="display: block; position: absolute; right: 5rem"
          >
            <button class="dropdown-item" @click="addFolder">Folder</button>
            <button class="dropdown-item" @click="addDocument">
              File Upload
            </button>
          </div>
        </div>
      ...
      >
        ...
        <p class="fs-4 fw-semibold text-center">No Files available.</p>
      </div>
          ...
          <div class="folder-dropdown position-absolute top-0 end-0 m-2">
            ...
            >
              <div
                ...
                @click="viewFileDetailsModal(doc.id)"
              ...
              >
                ...
                <div
                  ...
                  v-if="shareSubMenuIndex === index && shareType === 'document'"
                  @click.stop
                  @mouseenter="shareSubMenuIndex = index"
        ...
        >
          <div class="folder-dropdown position-absolute top-0 end-0 m-2">
            ...
            >
              ...
              <div
                class="folder-menu-item position-relative"
                @click.stop="toggleShareSubMenu(index, 'folder')"
                @mouseenter="shareSubMenuIndex = index"
              >
                Share
                <div
                ...
                >
                  <div
                    class="folder-menu-item"
                    @click="
                      toggleShowShareToPersonModal(folder.id, folder.type)
                    "
                  >
                    To a person
                  </div>
...
export default {
  ...
  methods: {
    ...
    async loadFolderContents() {
      ...
      try {
        if (folder_id) {
          ...
          this.filteredFolders = [...folderRes];
          this.filteredDocuments = [...documentRes];
        } else {
          this.folders = [];
          this.documents = [];
          this.filteredFolders = [];
    ...
    async fetchFolders(folder_id) {
      ...
      try {
        const response = await axios.get(
          `${process.env.VUE_APP_API_URL}/getSubfolders/${folder_id}`,
    ...
    async fetchDocuments(folder_id) {
      ...
      try {
        const response = await axios.get(
          `${process.env.VUE_APP_API_URL}/getFolderFiles/${folder_id}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
    ...
    async viewFileDetailsModal(doc_id) {
      const token = localStorage.getItem("token");
    ...
    // functions to handle renaming of folders
    ...
    async shareFolderToTeam() {
      ...
      await axios
        .post(
          `http://127.0.0.1:8000/api/shareFolderWithTeam/${this.folder_id}`,
          {
            granted_by: user_id,
            team_id: this.selectedTeamId,
            permission: this.permission,
